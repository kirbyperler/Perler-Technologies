...
function questionnaireLinkFor(token) {
  return `${siteBase()}/questionnaire/${token}`;
}
...
async function sendQuestionnaireInviteEmail(db, { questionnaire, recipientEmail, customMessage, type }) {
  const link = questionnaireLinkFor(questionnaire.accessToken);
...
async function sendCompletionNotificationEmail(db, { questionnaire, clientName, projectName }) {
  const adminLink = `${siteBase()}/admin#/questionnaires/${String(questionnaire._id)}`;
